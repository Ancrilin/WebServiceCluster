...
class Vocabulary(object):
    def __init__(self, graph):
        ...
        self._curr_id = 0
        print('遍历所有节点')
        for node in tqdm(graph.nodes()):#遍历所有节点
            if node not in self.dict_node2id:#如果该节点没有编号，加入
                self._curr_id += 1
                self.dict_node2id[node] = self._curr_id#加入到节点-id字典中
                self.dict_id2node[self._curr_id] = node#加入到id-节点字典中

    def id2node(self, id):
        return self.dict_id2node[id]

    def node2id(self, node):
        return self.dict_node2id[node]

    def augment(self, graph):#增加图
        for node in graph.nodes():
            if node not in self.dict_node2id:
                self._curr_id += 1
                self.dict_node2id[node] = self._curr_id
                self.dict_id2node[self._curr_id] = node

    def __len__(self):
        return self._curr_id

    def getnode2id(self):
        return self.dict_node2id
